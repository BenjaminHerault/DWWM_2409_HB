...
fetch("./assets/info.json")
    ...
    .then((data) => {
        ...
        ConnextionValider.addEventListener("click", function () {
            for (let i = 0; i < data.length; i++) {
                if (
                    utilisateurs[i] === utilisateursValider.value &&
                    motDePasses[i] === motDePasseValider.value
                ) {
                    Avalider = true;
...
function connextionReussi(objet, data) {
    ...
    texteConnextion.innerHTML =
        "<br><br>Bonjour " + objet.firstname + " " + objet.lastname;
...
function creactionDuTableau(data, utilisateurConnecte) {
    ...
    function uneAdresseMail(index) {
        ...
        let Email =
            lePrenom.toLowerCase() + "." + leNom.toLowerCase() + "@example.com";
        console.log(Email);
        return Email;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        let surbrillance =
            data[i].firstname === utilisateurConnecte.firstname &&
            data[i].lastname === utilisateurConnecte.lastname;
